#include "Form.hpp"
#include "Bureaucrat.hpp"

Form::Form(void) : name(""), ToSign(150), ToExec(150)
{
    ItIsSigned = false;
}

Form::~Form()
{
}

Form::Form(const Form &obj) : name(obj.name), ToSign(obj.ToSign), ToExec(obj.ToExec)
{
    ItIsSigned = obj.ItIsSigned;
}

Form &Form::operator=(const Form &obj)
{
	if (this == &obj)
		return (*this);
	this->ItIsSigned = obj.ItIsSigned;
	return (*this);
}

int Form::PreCheck(int val)
{
    if (val > 150)
       throw GradeTooLowException();
    else if (val < 1)
        throw GradeTooHighException();
	return (val);
}

Form::Form(const std::string &name, const bool &ItIsSigned, const int &Tosign, const int &Toexec) : name(name),
    ItIsSigned(ItIsSigned)
    , ToSign(PreCheck(Tosign))
    , ToExec(PreCheck(Toexec))
{
}


const std::string	&Form::getName()        const{return name;}
const bool 			&Form::getItIsSigned()  const{return ItIsSigned;}
const int			&Form::getToSign()      const{return ToSign;}
const int			&Form::getToExec()      const{return ToExec;}


std::ostream &operator<<(std::ostream &os, const Form &obj1)
{
	os << "Form Name        : " << obj1.getName() << std::endl;
    if (obj1.getItIsSigned())
    {
        os << "Status           : " << "Signed" << std::endl;
    }
    else
    {
        os << "Status           : " << "Not Signed" << std::endl;
    }
    os << "the sign grade   : " << obj1.getToSign() << std::endl;
    os << "the Execute grade: " << obj1.getToExec();

	return os;
}


void Form::beSigned(const Bureaucrat &bur)
{

	if (bur.getGrade() <= ToSign)
		ItIsSigned = true;
	else
		throw GradeTooLowException();
}